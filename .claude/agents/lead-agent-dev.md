...
- Toda função Postgres sensível (`entregar_leads`, `confirmar_compra`, etc.) tem `revoke execute ... from anon, authenticated, public` + `grant ... to service_role` — só o backend chama, nunca o cliente direto. **`anon` tem que estar explícito no revoke, não só `public`** — o Supabase concede `EXECUTE` a `anon` por padrão em toda function nova do schema `public` via `ALTER DEFAULT PRIVILEGES`, e esse grant é próprio do role, não vem de `PUBLIC`; revogar só de `public, authenticated` deixa a function exploitável por requisição sem login. Já aconteceu de verdade em produção com `confirmar_compra` (`SECURITY DEFINER`, ignorava RLS) — ver `CONTEXTO.md` seção 23.
...
- **Antes de criar uma branch nova, sempre rodar `git fetch kintekit --prune` e checar `git branch -a | grep -i "<número>"`** — o sócio costuma pré-criar (e às vezes já trabalhar de verdade em) branches pra histórias futuras. Branch vazia (`git diff main...kintekit/feature/X --stat` sem saída): geralmente presa num commit antigo da main — reaproveitar o nome mas atualizar o conteúdo: `git checkout -b feature/X kintekit/feature/X && git reset --hard main` (só depois de confirmar com `git log --oneline main..kintekit/feature/X` que não há nenhum commit único pra perder). Branch com trabalho real:
