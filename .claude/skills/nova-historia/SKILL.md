...
7. **Commit + merge + versão**: commit descritivo em português (o *porquê*, não só o *o quê*) → `git checkout main` → `git merge --no-ff feature/$numero-$nome` → bump de patch no `package.json`/`package-lock.json` (`npm version patch --no-git-tag-version` + `git commit -am "chore: bump versão para X.Y.Z"`), checando corrida com commits novos do sócio antes do push — passo a passo completo na seção "Versionamento do `package.json`" do agente `lead-agent-dev` — → `git push kintekit main` → `git push kintekit feature/$numero-$nome`. **Nunca** dar push em `origin`/Levartosky sem pedido explícito do usuário.
